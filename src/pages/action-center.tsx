        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-5 p-6">
          <div className="min-w-0">
            <p className="section-kicker">
              {active?.plan_summary
                ? "AUTO-ORCHESTRATED PLAN"
                : "HUMAN-LED RETENTION"}
            </p>
            <h2 className="mt-2 font-display text-xl font-semibold">{name}</h2>
            <p className="mt-1 text-sm text-muted-foreground">{role}</p>
            {active?.plan_summary && (
              <p className="mt-3 max-w-2xl rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs leading-relaxed text-muted-foreground">
                <strong className="text-foreground">Agent plan: </strong>
                {active.plan_summary}
              </p>
            )}
          </div>
